...
def RGB2HSV(bgr):
    ...
    print("1 Working time :", time.time() - start)

    r, g, b = (bgr[:,:,2]/255), (bgr[:,:,1]/255), (bgr[:,:,0]/255)
    print(" 2Working time :", time.time() - start)
    # cmax = np.maximum(np.maximum(r,g), b)
    # cmin = np.minimum(np.minimum(r,g), b)
    stacked_rgb = np.stack([r,g,b])
    cmax = np.max(stacked_rgb)
    cmin = np.min(stacked_rgb)
    print(cmax.shape)
    
    print("3 Working time :", time.time() - start)
    delta = (cmax - cmin)
    print("3 Working time :", time.time() - start)
      
    #calculate val,hue, sat, val>>어케 줄이지
    value = cmax * 255
    print("4 Working time :", time.time() - start)    
    # for i in range(height):
    #     for j in range(width):
    #         # if delta[i,j] == 0 :
    #         #     hue[i,j] = 0
    ...
    print("5 Working time :", time.time() - start)
    saturation = np.where(cmax!=0, (delta / cmax) * 255, 0)
    print("6Working time :", time.time() - start)
    ...
    print("7Working time :", time.time() - start)
    hsv_image[:, :, 0] = hue
    hsv_image[:, :, 1] = saturation
    hsv_image[:, :, 2] = value
    print("8Working time :", time.time() - start)
    hsv_image = hsv_image.astype(np.uint8)
    print("9Working time :", time.time() - start)
...
if __name__ == '__main__':
    ...
    # RGB to HSV 변환
    HSV = RGB2HSV(image)
    print("2HSV Working time :", time.time() - start)
    ...
    print("Mask Working time :", time.time() - start)
